...
internal class PagedStorage<T : Any> :
    ...
    NullPaddedList<T> {
    private val pages = mutableListOf<PagingSource.LoadResult.Page<*, T>>()
    ...
    constructor(
        leadingNulls: Int,
        page: PagingSource.LoadResult.Page<*, T>,
        trailingNulls: Int
    ) : this() {
    ...
    private inline fun <V> traversePages(
        localIndex: Int,
        crossinline onLastPage: (page: PagingSource.LoadResult.Page<*, T>, pageInternalIndex: Int) -> V
    ): V {
    ...
    fun getRefreshKeyInfo(@Suppress("DEPRECATION") config: PagedList.Config): PagingState<*, T>? {
        ...
        return PagingState(
            pages = pages.toList() as List<PagingSource.LoadResult.Page<Any, T>>,
    ...
    internal fun prependPage(page: PagingSource.LoadResult.Page<*, T>, callback: Callback? = null) {
    ...
    internal fun appendPage(page: PagingSource.LoadResult.Page<*, T>, callback: Callback? = null) {
        val count = page.data.size
        if (count == 0) {
            // Nothing returned from source, nothing to do
            return
