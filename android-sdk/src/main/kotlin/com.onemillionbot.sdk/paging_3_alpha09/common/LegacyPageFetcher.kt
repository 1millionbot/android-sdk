...
internal class LegacyPageFetcher<K : Any, V : Any>(
    ...
    private fun scheduleLoad(type: LoadType, params: PagingSource.LoadParams<K>) {
    ...
    private fun onLoadSuccess(type: LoadType, value: PagingSource.LoadResult.Page<K, V>) {
        ...
        if (pageConsumer.onPageResult(type, value)) {
            ...
            loadStateManager.setState(
                type,
                if (value.data.isEmpty()) LoadState.NotLoading.Complete else LoadState.NotLoading.Incomplete
            )
        }
    }
    ...
    fun trySchedulePrepend() {
        ...
        if (startState is LoadState.NotLoading && !startState.endOfPaginationReached) {
            schedulePrepend()
        }
    }
    ...
    fun tryScheduleAppend() {
        ...
        if (endState is LoadState.NotLoading && !endState.endOfPaginationReached) {
            scheduleAppend()
        }
    }
