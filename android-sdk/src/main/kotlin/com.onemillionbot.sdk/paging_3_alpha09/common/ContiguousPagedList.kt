...
open class ContiguousPagedList<K : Any, V : Any>(
    ...
    override fun onPageResult(
        ...
        if (shouldTrim) {
            ...
            if (trimFromFront) {
                if (pager.loadStateManager.startState !is LoadState.Loading) {
                    if (storage.trimFromFront(
                        ...
                        pager.loadStateManager.setState(LoadType.PREPEND, LoadState.NotLoading.Incomplete)
                    }
                }
            } else {
                if (pager.loadStateManager.endState !is LoadState.Loading) {
                    if (storage.trimFromEnd(
                        ...
                        pager.loadStateManager.setState(LoadType.APPEND, LoadState.NotLoading.Incomplete)
                    }
                }
            }
        }
    ...
    private fun triggerBoundaryCallback(type: LoadType, page: List<V>) {
        if (boundaryCallback != null) {
            ...
            val deferBegin = (!deferEmpty && type == LoadType.PREPEND && page.isEmpty())
            val deferEnd = (!deferEmpty && type == LoadType.APPEND && page.isEmpty())
